import React, { useState, useEffect } from 'react';
import { Feather } from '@expo/vector-icons';
import { View, Text, FlatList, Image, TouchableOpacity } from 'react-native';
import { useNavigation, useRoute } from '@react-navigation/native';
import { WebView } from 'react-native-webview';
import MapView from 'react-native-maps';
import { Marker } from 'react-native-maps';
import { Callout } from 'react-native-maps';
import * as Location from 'expo-location';
// import { MapContainer, TileLayer, Marker, Popup } from 'react-leaflet';

import api from '../../services/api';

import logoImg from '../../assets/logo.png';

import styles from './styles';
import colors from '../../styles/colors';

const reportIcons = {
  default: require('../../assets/ReportIcons/treeicon.png'),
  streetHole: require('../../assets/ReportIcons/streetHole.png'),
  eletricCable: require('../../assets/ReportIcons/eletricCable.png'),
  lightPole: require('../../assets/ReportIcons/lightPole.png'),
  treeIcon: require('../../assets/ReportIcons/treeicon.png') 
}

const getReportIcon = iconName => reportIcons[iconName] || reportIcons.default;

const reportPictures = {
  default: "data:image/jpeg;base64,",
  streetHole: "http://midia.gruposinos.com.br/_midias/jpg/2017/04/11/buraco-2980152.jpg",
  eletricCable: "https://br.habcdn.com/photos/questions/medium/6855edd4-2de8-484e-8f55-756670629c58-2239799.jpg",
  lightPole: "data:image/jpeg;base64,",
  treeIcon: "https://f088b146830a59b5.cdn.gocache.net/uploads/noticias/2020/10/14/khy3ornqq457.jpeg", 
}

const getReportPicture = pictureName => reportPictures[pictureName] || reportPictures.default;

export default function MainScreen(){
  const [location, setLocation] = useState(null);
  const [errorMsg, setErrorMsg] = useState(null);
  
  const [reports, setReports] = useState([]);
  const [userData, setUserData] = useState([{nome: 'loading'}]);

  // const user_id = '25cd9537';

  async function loadReports() {

    const response = await api.get('reports');    
  
    setReports(response.data);
  }

  async function loadUserInformation() {

    const response = await api.get('users');    
    
    // console.log(response.data);
    setUserData(response.data);
  }
  
  

  const navigation = useNavigation();
  const route = useRoute();

  function navigateToProfile(){
    navigation.navigate('Profile');
  }

  function navigateToReportCreator(){
    navigation.navigate('ReportCreator');
  }

  function navigateToReportSearch(){
    navigation.navigate('ReportSearch');
  }

  function navigateToReportInspect(report_id){
    console.log("navigate: ",report_id)
    navigation.navigate('ReportInspect', {report_id});

  }

  useEffect(() => {
    (async () => {
      let { status } = await Location.requestForegroundPermissionsAsync();
      if (status !== 'granted') {
        setErrorMsg('Permission to access location was denied');
        return;
      }

      let location = await Location.getCurrentPositionAsync({});
      setLocation(location);
    })();
    loadReports();
    loadUserInformation();
  }, []);

  let text = 'Waiting..';
  if (errorMsg) {
    text = errorMsg;
  } else if (location) {
    text = JSON.stringify(location);
  }

  let region;

  if(location === null){
    region = {
      latitude: 0.0,
      longitude: 0.0,
      latitudeDelta: 100,
      longitudeDelta: 100,
    }
  } else if (location !== null){
    region = {
      latitude: location.coords.latitude,
      longitude: location.coords.longitude,
      latitudeDelta: 0.015,
      longitudeDelta: 0.0121,
    }
  }


  // console.log(region);
  // console.log(reports);
  // console.log(reports[3].local.latitude);
  // descobrir como abstrair latitude e longitude do objeto/ descobrir como renderizar varios markers/ recarregar página ao voltar do profile
  
  return(
    <View style={styles.container}>
      <View style={styles.header}>
        <TouchableOpacity style={styles.profileTouchable} onPress={navigateToProfile}>
          <View style={styles.imageBorder}>
            <Image style={styles.profileImage} source={require('../../assets/profilePic.jpg')}/>
          </View>
          <Text style={styles.profileName}>{userData[0].nome}</Text>
        </TouchableOpacity>
        <Image source={logoImg} style={styles.logo} />
      </View>
      <View style={styles.mainContent}>
        <MapView
          style={styles.map}
          loadingEnabled={true}
          region={region}
          showsUserLocation={true}
          showsMyLocationButton={true}
        >
          {reports.map(report => (
            <Marker key={report.id} coordinate = {{latitude: Number(report.latitude), longitude: Number(report.longitude)}}
            image={getReportIcon(report.typeIcon)}>
              <Callout tooltip={true} onPress={() => navigateToReportInspect(report.id)}>
                <View style={styles.reportBox} key={report.id}>                
                  <View style={styles.report}>
                    <View style={styles.porpertyValueBlock}>
                      <Text style={styles.reportProperty}>Tipo:</Text>
                      <Text style={styles.reportValue}>{report.typeDescription}</Text>
                    </View>
                    <View style={styles.statusAndInspectButtonBox}>
                      <View style={[styles.porpertyValueBlock, { paddingTop: 10 }]}>
                        <Text style={styles.reportProperty}>Status:</Text>
                        <Text style={styles.reportValue}>{report.statusDescription}</Text>                      
                      </View>
                      <View style={styles.inspectButtonBox}>
                        <TouchableOpacity style={styles.inspectReportButton} onPress={navigateToReportInspect}>
                          <Text style={styles.inspectReportButtonText}>Detalhes</Text>
                        </TouchableOpacity>
                      </View>
                    </View>
                    <View style={styles.reportImageAndDescriptionBox}>
                      <View style={styles.reportImageBox}>     
                        <WebView style={styles.reportImage} source={{ uri: getReportPicture(report.typeIcon) }} />               
                      </View>                                                             
                        <Text style={styles.reportDescription}>{report.descricao}</Text>                  
                    </View>
                    <View style={styles.porpertyValueBlockOwner}>
                      <Text style={styles.reportOwnerProperty}>Reportado por:</Text>
                      <Text style={styles.reportOwnerName}>{report.nome}</Text>
                    </View>
                  </View>
                </View>
              </Callout>            
            </Marker>
          ))}
        </MapView>      
      </View>
      <View style={styles.footer}>
        <TouchableOpacity style={styles.createReportButton} onPress={navigateToReportCreator}>
          <Text style={styles.createReportButtonText}>+ Criar Report</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles.searchButton} onPress={navigateToReportSearch}>
        <Feather name="search" size={25} color={colors.white} style={{transform: [{scaleX: -1}]}}/>
          <Text style={styles.searchButtonText}>Pesquisar</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
}